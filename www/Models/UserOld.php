...
class UserOld
{
    ...
    public function createUser(string $firstname, string $lastname, string $username, string $email, string $password, string $role, string $ActivationToken): bool
    {
        $pdo = $this->database->getDatabaseConnection();

        $stmt = $pdo->prepare("INSERT INTO users (Firstname, Lastname,username, email, password, Role, is_validated, created_at) 
VALUES (?, ?, ?, ?, ?, ?, FALSE, NOW())");
        return $stmt->execute([$firstname, $lastname, $username, $email, $password, $role]);
    }
